    ...
    request: (req: any, callback: any) => {
      axios
        .get(req.url, {
          ...
          headers: { "TomTom-User-Agent": `TomTomMCPSDK/${VERSION}` },
        })
        ...
        .map((route: any, routeIndex: number) => {
          ...
          if (routePoints.length > 1) {
            ...
            if (validCoords.length > 1) {
              ...
              const trafficDelay =
                currentRouteData.trafficDelay || formatTime(trafficDelayInSeconds);
              const trafficColor =
                currentRouteData.trafficColor ||
                getTrafficColor(travelTimeInSeconds, trafficDelayInSeconds);
